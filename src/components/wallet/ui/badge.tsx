...
export function Badge({
  ...
  if (isPopular) {
    return (
      <div className="absolute -top-6 -right-6 z-50">
        <div
          className={`${bgColor} text-white font-bold text-sm px-6 py-2 rounded-lg shadow-lg border-2 border-white transform rotate-12`}
        >
          ⭐ {text} ⭐
        </div>
      </div>
  ...
  return (
    <div className="absolute top-4 right-4 z-10">
      <div
        className={`${bgColor} text-white font-semibold text-xs px-4 py-2 rounded-lg shadow-sm`}
      >
        {text}
      </div>
    </div>
